...
(async function () {
  ...
  logger.loader(` NAME BOT: ${(!global.config.BOTNAME) ? "This bot was made by Nino Kawai" : global.config.BOTNAME}`)
  logger.loader("┗━━━━━━━━━━━━━━━━━━━━━━━━━━┛")
...
//DEFINE DATLICH PATH
  ...
  return async (event) => {
    ...
    if (!find_thuebot)return send(`❎ Nhóm của bạn chưa thuê bot, vui lòng liên hệ Admin Ng Văn Chiến\n────────────────\nFacebook: fb.me//vanchiendzs`);
    if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now()+25200000)return send(`❎ Nhóm của bạn đã hết hạn thuê bot, vui lòng liên hệ Admin Ng Văn Chiến\n────────────────\nFacebook: fb.me//vanchiendzs`);
    };
